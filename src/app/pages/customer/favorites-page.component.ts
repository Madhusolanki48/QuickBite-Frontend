  ...
  template: `
    <section class="grid gap-4">
      <header class="rounded-[30px] border border-white/70 bg-white/75 p-4 shadow-glow backdrop-blur-xl md:p-5">
        <p class="text-xs font-extrabold uppercase tracking-[0.18em] text-brand">Favorites</p>
        <div class="mt-2 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
              A dedicated list of everything you’ve hearted in QuickBite.
            </p>
          </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <article *ngFor="let item of items(); trackBy: trackItem" class="group overflow-hidden rounded-[28px] border border-white/70 bg-white/85 shadow-soft transition hover:-translate-y-1 hover:shadow-glow">
            <div class="relative aspect-[4/3] overflow-hidden">
              ...
              <button
                type="button"
                class="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full bg-white/90 text-brand shadow-soft transition hover:-translate-y-0.5"
            ...
            <div class="grid gap-3 p-4">
              <div class="flex items-start justify-between gap-3">
                <div>
                  <h3 class="font-['Space_Grotesk'] text-base font-bold tracking-[-0.04em] text-ink">
                    {{ item.name }}
                  </h3>
      ...
      <ng-template #emptyState>
        <div class="rounded-[24px] border border-dashed border-slate-300 bg-white/70 p-6 text-sm text-slate-500">
...
export class FavoritesPageComponent {
  ...
  protected deliveryTime(item: MenuItem): string {
    return this.catalog.restaurantById(item.restaurantId)?.deliveryMinutes ?? '25 min';
  }

  protected remove(item: MenuItem, event: MouseEvent): void {
